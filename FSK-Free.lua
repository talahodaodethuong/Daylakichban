...
RunService.RenderStepped:Connect(function(dt)
    ESPManager._accum += dt
    if ESPManager._accum < 0.25 then return end
    ESPManager._accum = 0

    local root = getHRP()
    if not root then return end

    for model, data in pairs(ESPManager._updateList) do
        if not model or not model.Parent then
            ESPManager._updateList[model] = nil
            continue
        end
        ...
        if not label or not objRoot then continue end
...
function ESPManager:_ScheduleCreate(model, typeName)
    if not model or not self.ActiveTypes[typeName] then return end
...
Button.Image = "rbxassetid://122312005360431"
...
local Options = Fluent.Options
    ...
    Tabs.About:AddParagraph({
        Title = "Dev Script",
        Content = "• HTZ GAMING - Admin\n• NTK - Obfuscate\n• Krop VN - Find Remote\n• Shade Vex - Make Anti Ban\n• RimuruHRB - Make Gui\n• Yangho VN - Fix Bug"
    })
...
_G.DusekAimMode = _G.DusekAimMode or 1 -- 1 = killers, 2 = survivors, 3 = nearest
...
local function getTarget()
    ...
    lastScan = now
    local mode = _G.DusekAimMode

    if mode == 3 then
        local killers = getKillersFolder() and getKillersFolder():GetChildren() or {}
        local surv = getSurvivorsFolder() and getSurvivorsFolder():GetChildren() or {}
        local combined = {}

        for _,a in ipairs(killers) do table.insert(combined,a) end
        for _,b in ipairs(surv) do table.insert(combined,b) end

        cachedTarget = findNearest(combined)
        return cachedTarget
    end

    local folder = (mode == 1) and getKillersFolder() or getSurvivorsFolder()
...
local function ensureRender()
    ...
    renderLoop = Run.RenderStepped:Connect(function()
        if not isAiming or not _G.DusekAimEnabled then return end

        local anch = anchorCF()
        if not anch then return end

        local anchPos = anch.Position
        local t = getTarget()

        if t then
            Camera.CFrame = CFrame.new(anchPos, t.Position)
        else
            Camera.CFrame = CFrame.new(anchPos, anchPos + anch.LookVector * 15)
        end
    end)
end
...
if Tabs and Tabs.Main then
    ...
    if Tabs.Main.AddDropdown then
        Tabs.Main:AddDropdown("DusekAimModeDrop", {
            Title = "Aim Mode",
            Values = { "Aim Killers", "Aim Survivors", "Aim Nearest" },
            Default = (_G.DusekAimMode == 2 and "Aim Survivors")
                or (_G.DusekAimMode == 3 and "Aim Nearest")
                or "Aim Killers",
            Callback = function(v)
                _G.DusekAimMode = (v == "Aim Killers" and 1)
                    or (v == "Aim Survivors" and 2)
                    or 3
                cachedTarget = nil
            end
        })
    ...
    if Tabs.Main.AddToggle then
        Tabs.Main:AddToggle("DusekAimSkill", {
            Title = "Dusekkar Aimlock",
            Description = "You need to use ShiftLock",
            Default = _G.DusekAimEnabled,
            Callback = function(v)
...
Tabs.Misc:AddToggle("AutoClickActiveButton", {
    Title = "Auto Click ActiveButton",
    Default = false
}):OnChanged(function(state)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, v in ipairs(tempUI:GetDescendants()) do
        ...
        and not clickedButtons[v] then
            ...
            local cons = getconnections(v.MouseButton1Click)
...
InterfaceManager:SetFolder("Hutao Hub [Free]")
SaveManager:SetFolder("Hutao Hub [Free]/Forsaken")
