...
local Window = WindUI:CreateWindow({
    ...
    Author = "99 Night In The Forest | By: HTZ GAMING",
...
Tabs.Vision:Button({
    ...
    Callback = function()
        print("[HutaoHub] FPS Boost Applied")
        ...
        if success then
            print("[HutaoHub] Script loaded successfully")
        else
            warn("[HutaoHub] Failed to load script: " .. tostring(err))
            print("[HutaoHub] hi 1")
        end
    end
})
...
RunService.RenderStepped:Connect(function()
    ...
    if tick() - fpsLastUpdate >= 1 then
        ...
        if showPing then
            ...
            if ping > 120 then
                color, label = Color3.fromRGB(255, 0, 0), "So Cool Ping: "
            elseif ping > 60 then
